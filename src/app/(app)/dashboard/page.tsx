...
import { collection, doc, query, where, DocumentData, setDoc, serverTimestamp } from 'firebase/firestore';
import { db, auth } from '@/lib/firebase';
import { useDocumentData } from 'react-firebase-hooks/firestore';
import { useAuthState } from 'react-firebase-hooks/auth';

// Define the SpaceData interface
interface SpaceData {
  id: string;
  name: string;
  description?: string;
  ...
  rating?: number;
  location?: string;
  isRealSpace?: boolean; // Add this to distinguish real vs sample data
}

// Enhanced gradient utility function
...
export default function DashboardPage() {
  ...
  const realSpaces: SpaceData[] = allSpaces?.docs?.map(doc => ({
    id: doc.id,
    ...doc.data(),
    isRealSpace: true
  } as SpaceData)) || [];

  // Use real spaces if available, otherwise show empty state or sample spaces for demo
  const spacesToShow = realSpaces.length > 0 ? realSpaces : [];

  const activeSpaces = spacesToShow.filter(space => space.activeMeeting);
  const recentSpaces = spacesToShow.slice(0, 4);
            ...
            {userData?.role === 'admin' && (
              <CreateSpaceDialog>
                <Button size="lg" className="group relative overflow-hidden bg-gradient-to-r from-blue-600 via-purple-600 to-indigo-600 hover:from-blue-700 hover:via-purple-700 hover:to-indigo-700 shadow-xl hover:shadow-2xl transition-all duration-300 border-0 text-white font-semibold px-6 py-2.5">
                  ...
                  <span className="relative">Create New Space</span>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-purple-700 dark:text-purple-300">
                    {Array.from(new Set(spacesToShow.flatMap(s => s.members || []))).length}
                  </p>
                </div>
  ...
  />
      ...
      <section className="space-y-4">
        <div className="flex items-center justify-between">
          ...
          <Link href="/ai-suggestions" className="text-sm font-medium text-blue-600 hover:text-blue-500 flex items-center transition-colors">
...
// Extracted SpaceCard component for reusability
function SpaceCard({ 
  space, 
  isActiveMeeting, 
  formatLastActivity, 
  ...
  return (
    <Card className={`group hover:shadow-xl transition-all duration-300 relative overflow-hidden border-0 shadow-md ${
      isActiveMeeting ? 'hover:shadow-blue-100/50 dark:hover:shadow-blue-900/20' : 'hover:shadow-purple-100/50 dark:hover:shadow-purple-900/20'
    }`}>
      <CardHeader className="p-0">
        ...
        {/* Show warning badge for sample data */}
        {!space.isRealSpace && (
      ...
      <CardFooter className="p-4 pt-0">
        <Link href={`/space/${space.id}`} className="w-full">
          <Button 
            ...
            variant={isActiveMeeting ? 'default' : 'outline'}
            disabled={!space.isRealSpace} // Disable for demo data
          >
            {isActiveMeeting ? (
